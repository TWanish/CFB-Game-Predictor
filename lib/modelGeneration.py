...
def getModel(data):
        ...
        for i in range(0,len(data[team].values)):
            ...
            if type(team_week) is dict:
                try:
                    ...
                    for x_var in ['def_pass_yds', 'def_pen_yds', 'def_rush_yds', 
    ...
    for var in ['def_pass_yds', 'def_pen_yds', 'def_rush_yds', 'def_turnovers',
                'def_ypp']:
...
def getMLDF(data):
    ...
    def_srs = []
    ptsF =[]
    columns = ['off_ypp', 'off_turnovers', 'off_rush_yds', 'off_pen_yds', 
               'off_pass_yds', 'off_srs', 'def_ypp', 'def_turnovers',
               'def_rush_yds', 'def_pen_yds', 'def_pass_yds', 'def_srs',
               'ptsF']
    for team in data.columns.values:
        for i in range(0,len(data[team].values)):
            team_week = data[team].values[i]
            if type(team_week) is dict:
                try:
                    ...
                    off_rush_yds.append(float(data.loc['off_rush_yds',win_team]))
                    ...
                    def_rush_yds.append(float(data.loc['def_rush_yds',lose_team]))
                    ...
                    off_rush_yds.append(float(data.loc['off_rush_yds',lose_team]))
                    ...
                    def_rush_yds.append(float(data.loc['def_rush_yds',win_team]))
    ...
    df = pd.DataFrame(list(zip(off_ypp, off_turnovers, off_rush_yds, 
                               off_pen_yds, off_pass_yds, off_srs, def_ypp,
                               def_turnovers, def_rush_yds, def_pen_yds,
                               def_pass_yds, def_srs, ptsF)), columns=columns)
